...
import { CrimeDataTable, type CrimeIncident } from '@/components/organisms/CrimeDataTable';
import { AlertCenter, type AlertData } from '@/components/organisms/AlertCenter';
...
const INITIAL_ALERTS: AlertData[] = [
  {
    ...
    message: 'Downtown robbery frequency exceeds standard limits by 40% in past 12h.',
    severity: 'critical',
    timestamp: '15m ago',
    read: false,
  },
  {
    ...
    message: '911 response dispatch backlog in Sector 5 exceeds AAA target SLA metrics.',
    severity: 'high',
    timestamp: '45m ago',
    read: false,
  },
  {
    ...
    message: 'Repetitive burglary pattern matching grey sedan suspect in Sector 2.',
    severity: 'medium',
    timestamp: '2h ago',
    read: true,
  },
  {
    ...
    message: 'CCTV Analytics integration nodes verified operational in Sector 4.',
    severity: 'low',
    timestamp: '6h ago',
    read: true,
  },
...
export function DashboardPage() {
  ...
  // RTK Query hooks
  const { data: districtsMetrics = [], isLoading: isLoadingDistricts } = useGetDistrictsQuery();
  const { data: incidents = [], isLoading: isLoadingIncidents } = useGetIncidentsQuery();
  useGetRiskForecastsQuery();
  ...
  // Detail Dialog state
  const [selectedIncident, setSelectedIncident] = React.useState<CrimeIncident | null>(null);

  // Reset pagination on filter change
  React.useEffect(() => {
    setCurrentPage(1);
  }, [district, crimeType, startDate, endDate, selectedSeverities, searchQuery]);
  ...
  const handleRefresh = React.useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const newAlert: AlertData = {
        ...
        message: 'Aggregated analytics pipeline successfully synced with latest CAD log logs.',
        severity: 'low',
        timestamp: 'Just now',
        read: false,
      };
  ...
  const handleSeverityToggle = React.useCallback((sev: string) => {
  ...
  const handleDistrictChangeLocal = React.useCallback((val: string) => {
    dispatch(setDistrict(val === 'all' ? null : val));
  }, [dispatch]);

  const handleCrimeTypeChangeLocal = React.useCallback((val: string) => {
    dispatch(setCrimeTypes(val === 'all' ? [] : [val]));
  }, [dispatch]);

  const handleDateChangeLocal = React.useCallback((field: 'start' | 'end', val: string) => {
    dispatch(
      setDateRange({
    ...
    return incidents.filter((inc) => {
      ...
      if (globalFilters.district) {
        const matchesDistrict = inc.district
          ...
          : inc.location.toLowerCase().includes(globalFilters.district.toLowerCase());
      ...
      // Date bounds
      if (globalFilters.dateRange.start && inc.timestamp < globalFilters.dateRange.start) return false;
      if (globalFilters.dateRange.end && inc.timestamp > `${globalFilters.dateRange.end} 23:59`) return false;
  ...
  const avgResolutionRate = React.useMemo(() => {
    ...
    const total = districtsMetrics.reduce((sum, d) => sum + d.resolutionRate, 0);
    ...
    <DashboardLayout title="Dashboard">
      <div className="space-y-8 max-w-7xl mx-auto pb-12 px-1">
        ...
        <div className="space-y-3 pt-2">
          <div>
            <Typography variant="heading-md" className="font-semibold text-foreground">
              Karnataka Geospatial Workspace
            </Typography>
            <Typography variant="body-sm" color="muted">
              Interactive incident mapping, temporal playback control, and predictive hotspot analysis.
            </Typography>
          </div>
          {isLoadingIncidents || isLoadingDistricts ? (
            <div className="w-full h-[700px] md:h-[75vh] bg-slate-900/40 border border-border rounded-lg flex flex-col items-center justify-center space-y-4 animate-pulse">
              ...
              <div className="text-muted-foreground text-sm font-medium">Loading geospatial command canvas...</div>
            </div>
          ) : (
        ...
        <div className="space-y-3 pt-2">
          <div>
            ...
            <Typography variant="body-sm" color="muted">
              Detailed breakdown of active, investigating, and resolved emergency cases.
            </Typography>
          </div>
          <CrimeDataTable
        ...
        <Sheet open={isFiltersOpen} onOpenChange={setIsFiltersOpen}>
          <SheetContent side="left" className="w-[360px] sm:max-w-[360px] border-r border-border bg-slate-950 p-0 flex flex-col h-full">
            <SheetHeader className="border-b border-border p-4 bg-card/10">
              <SheetTitle className="font-semibold text-lg">Filter Parameters</SheetTitle>
              <SheetDescription>Refine geospatial and incident metrics</SheetDescription>
            </SheetHeader>
        ...
        <Sheet open={isAlertsOpen} onOpenChange={setIsAlertsOpen}>
          <SheetContent side="right" className="w-[400px] sm:max-w-[400px] border-l border-border bg-slate-950 p-0 flex flex-col h-full">
            <SheetHeader className="border-b border-border p-4 bg-card/10">
              <SheetTitle className="font-semibold text-lg">Alert Center</SheetTitle>
              <SheetDescription>Real-time tactical anomalies & notifications</SheetDescription>
            </SheetHeader>
              ...
              <>
                <DialogHeader>
                  <div className="flex items-center gap-2.5">
                    <Badge variant={selectedIncident.severity === 'critical' ? 'risk-critical' : selectedIncident.severity === 'high' ? 'risk-high' : 'secondary'} size="sm">
                      {selectedIncident.severity}
                    </Badge>
                    <DialogTitle className="font-data font-bold">{selectedIncident.caseNumber}</DialogTitle>
                  </div>
                </DialogHeader>

                <div className="space-y-4 pt-2">
                  <div>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Incident Type</span>
                    <Typography variant="body-md" className="font-semibold text-foreground capitalize">
                      {selectedIncident.type}
                    </Typography>
                  </div>

                  <div>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Date & Timestamp</span>
                    <Typography variant="body-sm" color="muted" className="font-data">
                      {selectedIncident.timestamp}
                    </Typography>
                  </div>

                  <div>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Location Address</span>
                  ...
                  <div>
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Case Brief / Narrative</span>
                    <Typography variant="body-sm" color="muted" className="leading-relaxed pt-1 border p-2.5 rounded-md bg-muted/15 border-border">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-0.5">
                      ...
                      <Badge variant={selectedIncident.status === 'resolved' ? 'success' : 'warning'} dot size="sm">
                        {selectedIncident.status}
                      </Badge>
                    </div>
                    <div className="space-y-0.5 text-right">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block">Assigned Unit</span>
