...
import {
  ...
  type NetworkEdge,
  type NetworkNode,
} from "@/features/network/data/mockNetworkData";
export type {
  NetworkNode,
  NetworkEdge,
} from "@/features/network/data/mockNetworkData";
...
export const networkApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getNetworkGraph: builder.query<NetworkGraph, NetworkFilters | void>({
      queryFn: (filters) => {
        ...
        if (filters?.search) {
          ...
          filteredNodes = filteredNodes.filter(
            (n) =>
              ...
              (n.properties?.alias &&
                String(n.properties.alias).toLowerCase().includes(searchLower)),
          );
        }
        ...
        if (filters?.riskLevel && filters.riskLevel !== "all") {
          filteredNodes = filteredNodes.filter((n) => {
            ...
            if (filters.riskLevel === "high")
              return n.riskScore >= 50 && n.riskScore < 80;
            if (filters.riskLevel === "medium")
              return n.riskScore >= 25 && n.riskScore < 50;
        ...
        if (filters?.minConnections) {
          filteredNodes = filteredNodes.filter(
            (n) => n.connections >= (filters.minConnections || 0),
          );
        }
    ...
    getNetworkClusters: builder.query<NetworkCluster[], void>({
      queryFn: () => {
        const clusters: NetworkCluster[] = [
          {
            id: "syn-1",
            name: "Bengaluru Cyber Fraud Network",
            nodeCount: 35,
            riskLevel: "critical",
            keyNodes: ["susp-1", "susp-6", "susp-11", "susp-16"],
          },
          {
            id: "syn-2",
            name: "Mysuru Vehicle Theft Ring",
            nodeCount: 28,
            riskLevel: "high",
            keyNodes: ["susp-2", "susp-7", "susp-12", "susp-17"],
          },
          {
            id: "syn-3",
            name: "Belagavi Smuggling Network",
            nodeCount: 22,
            riskLevel: "medium",
            keyNodes: ["susp-3", "susp-8", "susp-13", "susp-18"],
          },
          {
            id: "syn-4",
            name: "North Karnataka Drug Distribution Group",
            nodeCount: 18,
            riskLevel: "high",
            keyNodes: ["susp-4", "susp-9", "susp-14", "susp-19"],
          },
          {
            id: "syn-5",
            name: "Coastal Narcotics Syndicate",
            nodeCount: 12,
            riskLevel: "low",
            keyNodes: ["susp-5", "susp-10", "susp-15", "susp-20"],
          },
        ];
    ...
    getGlobalNetworkGraph: builder.query<
      GlobalNetworkGraphResponse,
      GlobalNetworkGraphParams | void
    >({
      query: (params) => ({
    ...
    >({
      queryFn: ({ sourceId, targetId }) => {
        ...
        if (foundPath) {
          ...
          const explanation =
            `Shortest connection established through ${foundPath.path.length - 1} hops: ` +
            pathSteps.join(" ➔ ");
    ...
    getSharedPhones: builder.query<SharedPhone[], { syndicateId: string }>({
      queryFn: ({ syndicateId: _syndicateId }) => {
        const data: SharedPhone[] = [
          {
            phone: "+91 98450 10012",
            suspects: ["Sunil Gowda", "Guru Hegde"],
            riskScore: 88,
            confidenceScore: 94,
            threatLevel: "High",
          },
          {
            phone: "+91 98450 10034",
            suspects: ["Vijay Patil", "Anil Shetty"],
            riskScore: 92,
            confidenceScore: 88,
            threatLevel: "Critical",
          },
          {
            phone: "+91 98450 10056",
            suspects: ["Sandeep Kumar", "Deepak Swamy"],
            riskScore: 68,
            confidenceScore: 72,
            threatLevel: "Medium",
          },
    ...
    getSharedVehicles: builder.query<SharedVehicle[], { syndicateId: string }>({
      queryFn: ({ syndicateId: _syndicateId }) => {
        const data: SharedVehicle[] = [
          {
            vehicle: "KA-01-ME-1002",
            suspects: ["Sunil Gowda", "Vijay Patil"],
          ...
          {
            vehicle: "KA-03-ME-1008",
            suspects: ["Guru Hegde", "Manoj Naik"],
          ...
          {
            vehicle: "KA-02-ME-1015",
            suspects: ["Sandeep Kumar", "Anil Shetty"],
            model: "Maruti Swift",
            riskScore: 42,
            confidenceScore: 60,
            threatLevel: "Low",
     ...
     */
    getCommonAssociates: builder.query<
      CommonAssociate[],
      { syndicateId: string }
    >({
      queryFn: ({ syndicateId: _syndicateId }) => {
        const data: CommonAssociate[] = [
          ...
          {
            associate: "Deepak Swamy",
            suspectsCount: 2,
            role: "Transit Liaison",
            riskScore: 54,
            confidenceScore: 70,
            threatLevel: "Medium",
          },
    ...
    detectCriminalCommunities: builder.query<CommunityDetail[], void>({
      queryFn: () => {
        const data: CommunityDetail[] = [
          {
            communityId: 0,
            name: "Bengaluru Cyber Core",
            memberCount: 78,
            primaryCrime: "Cybercrime/Extortion",
            riskScore: 82,
          },
          {
            communityId: 1,
            name: "Mysuru-Hassan Transit Node",
            memberCount: 65,
            primaryCrime: "Vehicle Theft/Smuggling",
            riskScore: 76,
          },
          {
            communityId: 2,
            name: "Belagavi border Smugglers",
            memberCount: 52,
            primaryCrime: "Narcotics/Smuggling",
            riskScore: 90,
          },
          {
            communityId: 3,
            name: "North Range Distribution Hub",
            memberCount: 45,
          ...
          {
            communityId: 4,
            name: "Coastal Narcotics Corridor",
            memberCount: 38,
            primaryCrime: "Coastal Smuggling",
            riskScore: 84,
          },
        ];
        return { data };
     ...
     */
    buildNetworkGraph: builder.mutation<
      CrimeNetworkGraphResponse,
      CrimeNetworkGraphRequest
    >({
      query: (body) => ({
